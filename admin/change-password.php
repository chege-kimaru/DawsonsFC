                    ...
                    <div class="panel-body">
                        <form role="form" id="changePasswordForm">
                            <input class="form-control" type="hidden"
                                   name="username" value="<?php print $_SESSION['username'] ?>">
...
<script>
    $(document).ready(() => {
        $('#changePasswordForm').submit(e => {
            e.preventDefault();
            $('#requestOverlay').show();
            axios.post(`http://localhost/DawsonsFc/api/admin.php?type=change-password`, new FormData($('#changePasswordForm')[0]))
                .then(res => {
                    $('#requestOverlay').hide();
                ...
                .catch(e => {
                    $('#requestOverlay').hide();
                    console.error(e);
                    alert('Could not chnage password');
                })
        });
    });
</script>
