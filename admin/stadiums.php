...
<script type="text/javascript">
    ...
    const stadiumTemplate = (stadium) => {
        return `
            <tr data-stadium="${encodeURIComponent(JSON.stringify((stadium)))}" onclick="setStadium(this)">
                <td><img style="width: 70px;" src="${'http://localhost/DawsonsFC/' + stadium.image}" alt="Stadium"></td>
                <td>${stadium.name}</td>
    ...
    const deleteStadium = (stadiumId) => {
        ...
        if (confirm('Are you sure you want to delete this stadium?')) {
            axios.delete(`http://localhost/DawsonsFc/api/stadium.php?id=${stadiumId}`)
                .then(res => {
                    $('#requestOverlay').hide();
                    setStadiums();
                })
    ...
    const submitStadium = (data) => {
        $('#requestOverlay').show();
        axios.post(`http://localhost/DawsonsFc/api/stadium.php`, data)
            .then(res => {
            ...
            .catch(e => {
                $('#requestOverlay').hide();
                console.error(e);
                alert('could not update stadium');
            })
    };
