...
<script type="text/javascript">
    ...
    const userTemplate = (user) => {
        ...
        switch (+user.role) {
            ...
            case 2:
                role = 'ADMIN';
                buttonTemplate = `<button onclick="changeRole('${user.id}', 3)" class="btn btn-danger"><i class="fa fa-pencil"></i> Demote</button>`;
                break;
            case 3:
                role = 'USER';
    ...
    const setUsers = () => {
        $('#requestOverlay').show();
        axios.get('http://localhost/DawsonsFc/api/admin.php')
            .then(res => {
    ...
    const changeRole = (userId, role) => {
        ...
        if (confirm('Are you sure you want to change this user\'s role?')) {
            ...
            axios.post(`http://localhost/DawsonsFc/api/admin.php?type=change-role`, formData)
                .then(res => {
                    $('#requestOverlay').hide();
                    setUsers();
                })
